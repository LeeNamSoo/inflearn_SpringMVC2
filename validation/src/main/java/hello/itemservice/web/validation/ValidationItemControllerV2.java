...
public class ValidationItemControllerV2 {
    ...
    //custom validator를 사용(주입)하기 위한 코드, controller가 호출될 때마다 실행된다.
   @InitBinder
...
//        bindingResult : 반드시 @ModelAttribute다음에 위치해야하며, 자동으로 ModelAttribute에 들어간다.
        ...
        log.info("target={}",bindingResult.getTarget());
        //검증로직
        //ValidationUtils.rejectIfEmptyOrWhitespace(bindingResult,"itemName","required"); <-간단한 공백검사를 다음과 같이 할 수 있다.
        if(!StringUtils.hasText(item.getItemName())){
//            bindingResult.addError(new FieldError("item","itemName", item.getItemName(), false, new String[]{"required.item.itemName"}, null, null));
...
//    @PostMapping("/add")
    ...
    //주입된 validator를 사용하는 버젼 @Validated를 사용하면 자동으로 itemValidator의 validate가 실행됨
    @PostMapping("/add")
